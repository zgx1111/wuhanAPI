<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:91:"/www/server/apache/htdocs/api/public/../application/admin/view/admin/adminGetWorkerAll.html";i:1575620526;s:82:"/www/server/apache/htdocs/api/public/../application/admin/view/public/_footer.html";i:1571402714;}*/ ?>
<!DOCTYPE HTML>
<html>
<head>
    <meta charset="utf-8">
...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 推广员 <span class="c-gray en">&gt;</span> 查看推广员 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
...
<script type="text/javascript">
    ...
    function getProducts(page){
        ...
        //alert(token);
        var params={
            url:'/eworker/adminGetEworkerAll',
            data:{'page':page},
            tokenFlag:true,
            type:'post',
            ...
            if(len>0) {
                for (var i = 0; i < len; i++) {
                    ...
                    str += '<tr class="text-c">' +
                        ...
                        '<td>￥' + item.truename + '</td>' +
                        '<td>' + item.phone+ '</td>' +
                        '<td width="200px"><button class="btn btn-success" onclick="selStore('+item.id+')">查看名下门店</button></td>' +
                        '<td width="200px"><button class="btn btn-primary" onclick="selOrder('+item.id+')">查看销售记录</button></td>' +
                        '<td width="200px"><button class="btn btn-primary" onclick="delEworker('+item.id+')">删除</button></td>' +
                        '</tr>';
                }
            }
            return str;
    ...
    function getCount(nowPage){
        var params={
            url:'/eworker/adminGetEworkerCount',
            data:{},
            tokenFlag:true,
            type:'post',
            sCallback:function(res) {
    ...
    function selStore(id){
        //alert(id);
        window.location.href = 'selStore?id='+id;
    }
    //查看销售记录
    function selOrder(id){
        window.location.href = 'adminGetEworkerOrder?id='+id;
    }
    //删除推广员
    function delEworker(id){
        layer.confirm('是否确认删除id为'+id+'的推广员,先确认是否有绑定门店！', ['确定','取消'], function(){
            var params={
                url:'/eworker/adminDelEworker',
                data:{'id':id},
                tokenFlag:true,
                type:'post',
                sCallback:function(res) {
